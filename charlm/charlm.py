...
class CharLM:
        ...
        for fan_in, fan_out in zip(neurons_per_layer, neurons_per_layer[1:]):
            ...
            if weights_biases_dbn == "normal":
                initialized_weights = torch.randn(
                    (fan_in, fan_out), dtype=torch.float32)*normalization_factor*(1-zero_out_weights)
                initialized_biases = torch.randn(fan_out, dtype=torch.float32)*(1-zero_out_biases)
            elif weights_biases_dbn == "uniform":
                initialized_weights = torch.rand(
                    (fan_in, fan_out), dtype=torch.float32)*normalization_factor*(1-zero_out_weights)
                initialized_biases = torch.rand(fan_out, dtype=torch.float32)*(1-zero_out_biases)
    ...
    def fit(self, 
        ...
        Args:
            ...
            normalize_layer (list of bool): List indicating whether to apply normalization for each layer.
            ...
            normalize_pre_activation (bool, optional): If True, applies normalization before activation functions.
                If False, applies normalization after activation functions. Defaults to True.
        ...
        embeddings = torch.randn((len(self.idx_to_char), 
                                 size_of_embeddings), requires_grad = True, dtype=torch.float32)
        ...
        normalization_parameters = [torch.tensor([], requires_grad=True)]*len(weights) # This list will store the gamma and beta parameters for each layer
        running_means_stds = [torch.tensor([])]*len(weights)
        for idx, layer in enumerate(normalize_layer):
            if layer:
                normalization_parameters[idx] = torch.cat((torch.ones((1, neurons_per_layer[idx+1]), dtype=torch.float32),
                                                          torch.zeros((1, neurons_per_layer[idx+1]), dtype=torch.float32)),
        ...
        for _ in tqdm(range(epochs)): # Loop over the number of epochs
            ...
            for n_layer, (weight, bias, activation, normalization_parameter) in enumerate(zip(
                ...
                if (normalize_layer[n_layer]) & (normalize_pre_activation):
                    gamma = normalization_parameter[0]
                    beta = normalization_parameter[1]
                    mean_batch = A.mean(dim=0, keepdim=True)
                    std_batch = A.std(dim=0, keepdim=True)
                    running_means_stds[n_layer][0] = running_means_stds[n_layer][0]*(1-0.001) + mean_batch*0.001
                    running_means_stds[n_layer][1] = running_means_stds[n_layer][1]*(1-0.001) + std_batch*0.001
                    A = ((A - mean_batch)/(std_batch + 1e-5))*gamma + beta  
                ...
                if activation == "softmax":
                    ...
                    if (normalize_layer[n_layer]) & (not normalize_pre_activation):
                        ...
                        running_means_stds[n_layer][0] = running_means_stds[n_layer][0]*(1-0.001) + mean_batch*0.001
                        running_means_stds[n_layer][1] = running_means_stds[n_layer][1]*(1-0.001) + std_batch*0.001
                        A_activation = ((A_activation - mean_batch)/(std_batch + 1e-5))*gamma + beta
        ...
        self.running_means_stds = running_means_stds
        self.normalize_layer = normalize_layer
        self.normalize_pre_activation = normalize_pre_activation
    ...
    def calculate_loss(self, X, y):
        ...
        with torch.no_grad():
            ...
            for weight, bias, activation, running_means_stds, normalize_layer, normalization_parameter in zip(self.final_weights, 
                                                                    self.final_biases, 
                                                                    self.activations,
                                                                    self.running_means_stds,
                                                                    self.normalize_layer,
                                                                    self.final_normalization_parameters):
                A = inputs_to_next_layer @ weight + bias
                if self.normalize_pre_activation & normalize_layer:
                    gamma = normalization_parameter[0]
                    beta = normalization_parameter[1]
                    A = ((A - running_means_stds[0])/running_means_stds[1])*gamma + beta
                activation_function = getattr(A, activation)
                if activation == "softmax":
                    A_activation = activation_function(dim=1)
                else:
                    A_activation = activation_function()
                    if (not self.normalize_pre_activation) & normalize_layer:
                        gamma = normalization_parameter[0]
                        beta = normalization_parameter[1]
                        A_activation = ((A_activation - running_means_stds[0])/running_means_stds[1])*gamma + beta
